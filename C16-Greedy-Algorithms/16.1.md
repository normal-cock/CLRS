...
### `Answer`
活动对教室的使用其实是申请和释放，很像连接池。通过按照时间顺序模拟所有活动对于教室的申请和释放来计算所需的最小教室数量。如果申请和释放同时发生，则优先释放。[查看代码](./16.1-3/find_fewest_halls.py)。  
具体的调度方式为：按照开始时间来模拟活动的发生，在该模拟过程中，每次活动发生时申请到的教室即为该活动要被安排的教室。  
