...
老的答案不正确。例如上面的有向图，先从b开始dfs，再从a开始dfs，既满足没有`foward edges`，也满足没有`cross edges in the same component`，但这个图并不是`singly connected`。老答案如下：
...
正确答案为对图进行dfs，如果满足如下三个条件，则是`singly connected`。

* 没有`foward edges`
* 没有`cross edges in the same component`
* 任意两个component不存在两个或两个以上的`cross edges`
